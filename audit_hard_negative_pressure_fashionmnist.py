...
def build_probe_loaders(data_root, probe_size, probe_seed, batch_size, num_workers, test_tfm):
    pin_memory = torch.cuda.is_available()
    train_probe_ds = datasets.FashionMNIST(
        root=data_root, train=True, transform=test_tfm, download=True
    )
    test_probe_ds = datasets.FashionMNIST(
        root=data_root, train=False, transform=test_tfm, download=True
    )
...
def train_and_audit(args):
    ...
    if output_path is None:
        output_path = f"results/audit_fashionmnist_{loss_name}_seed{args.seed}.jsonl"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Audit hard-negative pressure during Deep LDA training on Fashion-MNIST."
    )
